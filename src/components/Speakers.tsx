...
const speakers: Speaker[] = [
  {
    ...
    image: 'https://tmf25.marketing.uz/uploads/speakers/74e9ca5c-9002-4b1f-80f7-1154a120fa5f.webp',
    label: 'jury',
  },
  {
    id: '2',
...
const Speakers: React.FC = () => {
  ...
  return (
    <section id="speakers" className="py-12 sm:py-16 md:py-20 lg:py-24 bg-black border-t border-white/20 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 sm:mb-10 md:mb-12 gap-4 sm:gap-5 md:gap-6">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-7xl font-black font-heading text-white uppercase tracking-tighter leading-none break-words">
              ЖЮРИ <span className="text-slate-700">/</span> СПИКЕРЫ <span className="text-slate-700">/</span> МЕНТОРЫ
            </h2>
          </div>
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black to-transparent pt-20">
                       ...
                       <>
                         {Array.isArray(speaker.label) ? (
                           speaker.label.map((lbl, idx) => (
                             <span key={idx} className="text-xs font-mono text-slate-500 border border-slate-700 px-2 py-0.5 uppercase tracking-wider">
                               {lbl}
                             </span>
                           ))
                         ) : (
                ...
                <div className="mt-4 pt-4 border-t border-white/10 opacity-60 group-hover:opacity-100 transition-opacity">
                   ...
                   {speaker.topic && (
                     <p className="text-slate-500 text-xs mt-1">Тема: {speaker.topic}</p>
                   )}
                </div>
              </div>
